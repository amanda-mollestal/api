...
import { IHabit, HabitModel, MySchema } from '../models/HabitModel'

/**
 * Encapsulates a Mongoose repository base.
 */
export class MongooseRepositoryBase<T extends IHabit & Document> {
  /**
   * The Mongoose model.
   *
   * @type {mongoose.Model}
  ...
  get allowedModelPropertyNames() {
    if (!this.#allowedModelPropertyNames) {
      ...
      this.#allowedModelPropertyNames = Object.freeze(
        Object.keys((this.#model.schema as MySchema<T>).tree)
  ...
  async update(id: string, updateData: any, options: any) {
  ...
  async replace(id: string, replaceData: any, options: any) {
